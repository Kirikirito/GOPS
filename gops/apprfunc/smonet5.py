...
class TauLayers(nn.Module):
    def __init__(self, in_features: int, out_features: int, seq_len, layer_num: int =2, hidden_dim: int = 64, layer_norm=False,scale = 10, bias=-3) -> None:
        ...
        self.real_in_features = in_features*(seq_len//2+2)
        sizes = [self.real_in_features] + [hidden_dim] * (layer_num - 1) + [out_features]
        self.activation = nn.Sigmoid()
        self.layers = mlp(sizes, nn.ReLU, nn.Identity)
    ...
    def cal_feature(self, x: torch.Tensor) -> torch.Tensor:
        amp = torch.fft.rfft(x, dim=1, norm = 'forward').abs()
        feature = torch.cat((x[:,-1:,:],amp), dim=-2) # B,L+1,D
        feature = feature.reshape(-1, self.real_in_features)
        return feature
