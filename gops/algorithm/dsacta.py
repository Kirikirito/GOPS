...
__all__=["ApproxContainer","DSACTA"]
import time
...
class DSACTA(AlgorithmBase):
    """Modified DSAC algorithm without adjust gradient
    ...
    def __compute_gradient(self, data: DataDict, iteration: int):
        ...
        tb_info = {
            ...
            tb_tags["loss_critic"]: loss_q.item(), # NOTE: the value is not the true loss value
    ...
    def __compute_loss_q(self, data: DataDict):
        ...
        target_q1,target_q1_sample ,target_q1_bound = self.__compute_target_q(
            rew,
            done,
            q1.detach(),
        ...
        target_q2, target_q2_sample ,target_q2_bound = self.__compute_target_q(
            rew,
            done,
            q2.detach(),
        ...
        q1_loss = (torch.pow(self.mean_std1, 2) + bias) * torch.mean(
            -(target_q1_sample - q1).detach() / ( torch.pow(q1_std_detach, 2)+ bias)*q1
        ...
        q2_loss = (torch.pow(self.mean_std2, 2) + bias)*torch.mean(
            -(target_q2_sample - q2).detach() / ( torch.pow(q2_std_detach, 2)+ bias)*q2
    ...
    def __compute_target_q(self, r, done, q,q_std, q_next, q_next_sample, log_prob_a_next):
        ...
        return target_q.detach(),target_q_sample.detach(), target_q_bound.detach()
