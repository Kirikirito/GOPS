...
__all__=["ApproxContainer","DSACTS"]
import time
...
class DSACTS(AlgorithmBase):
    ...
    def __compute_gradient(self, data: DataDict, iteration: int):
        ...
        loss_q, q1, std1, min_std1 = self.__compute_loss_q(data)
        loss_q.backward()
    ...
    def __compute_loss_q(self, data: DataDict):
        ...
        return q1_loss, q1.detach().mean(), q1_std.detach().mean(), q1_std.min().detach()
    ...
    def __compute_loss_policy(self, data: DataDict):
        ...
        loss_policy = (self.__get_alpha() * new_log_prob - q1).mean()
