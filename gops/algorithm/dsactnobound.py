...
__all__=["ApproxContainer","DSACTNOBOUND"]
import time
...
class DSACTNOBOUND(AlgorithmBase):
    ...
    def __compute_loss_q(self, data: DataDict):
        ...
        q1_loss = weight*torch.mean(
            ...
            +(torch.pow(q1.detach() - target_q1, 2) / (2 * torch.pow(q1_std, 2))
            + torch.log(q1_std))
        )
        ...
        q2_loss = weight*torch.mean(
            ...
            + (torch.pow(q2.detach() - target_q2, 2) / (2 * torch.pow(q2_std, 2))
            + torch.log(q2_std))
        )
