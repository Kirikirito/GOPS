...
__all__ = ["ApproxContainer", "DSACSTDC"]
...
class DSACSTDC(AlgorithmBase):
    ...
    def __compute_gradient(self, data: DataDict, iteration: int):
        ...
        loss_q, q, std, min_std = self.__compute_loss_q(data)
        loss_q.backward()
        ...
        tb_info = {
            ...
            "DSAC/critic_avg_min_std-RL iter": min_std.item(),
    ...
    def __compute_loss_q(self, data: DataDict):
        ...
        q_std_clip = torch.clamp(q_std, min=0.1**0.5).detach()
        
        q_loss = torch.mean(
            -(target_q - q).detach() / (torch.pow(q_std_clip, 2))*q
                -(torch.pow(q.detach() - target_q_bound, 2) / (torch.pow(q_std_clip, 3))
                - 1/q_std_clip)*q_std
            )

        return q_loss, q.detach().mean(), q_std.detach().mean(), q_std.min().detach()
