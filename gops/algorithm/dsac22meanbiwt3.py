...
__all__=["ApproxContainer","DSAC22MEANBIWT3"]
import time
...
class DSAC22MEANBIWT3(AlgorithmBase):
    ...
    def __compute_loss_q(self, data: DataDict):
        ...
        std=1/2 *(q1_std+q2_std)

        self._td_bound = (1 - self.tau_b) * self._td_bound + self.tau_b * 3 * torch.mean(std)
