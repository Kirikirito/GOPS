...
__all__=["ApproxContainer","DSAC22MEANC"]
import time
...
class DSAC22MEANC(AlgorithmBase):
    ...
    def __compute_loss_q(self, data: DataDict):
        ...
        weight = 1.0
        q1_std_clip = torch.clamp(q1_std, min=0.1**0.5).detach()
        q2_std_clip = torch.clamp(q2_std, min=0.1**0.5).detach()


        q1_loss = weight*torch.mean(
            -(target_q1 - q1).detach() / ( torch.pow(q1_std_clip, 2))*q1
            -(torch.pow(q1.detach() - target_q1_bound, 2) / (torch.pow(q1_std_clip, 3))
            - 1/q1_std_clip)*q1_std
        )

        q2_loss = weight*torch.mean(
            -(target_q2 - q2).detach() / ( torch.pow(q2_std_clip, 2))*q2
            -(torch.pow(q2.detach() - target_q2_bound, 2) / (torch.pow(q2_std_clip, 3))
            - 1/q2_std_clip)*q2_std
        )
