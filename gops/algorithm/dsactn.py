...
__all__=["ApproxContainer","DSACTN"]
import time
...
class DSACTN(AlgorithmBase):
    ...
    def _compute_gradient(self, data: DataDict, iteration: int):
        ...
        q1_grad_norm = torch.norm(torch.cat([p.grad.flatten() if p.grad is not None else torch.zeros_like(p).flatten() for p in self.networks.q1.parameters()]))
        q2_grad_norm = torch.norm(torch.cat([p.grad.flatten() if p.grad is not None else torch.zeros_like(p).flatten() for p in self.networks.q2.parameters()]))
        policy_grad_norm = torch.norm(torch.cat([p.grad.flatten() if p.grad is not None else torch.zeros_like(p).flatten() for p in self.networks.policy.parameters()]))
        tb_info = {
    ...
    def _compute_loss_q(self, data: DataDict):
        ...
        # form5
        q1_loss = torch.mean((2*huber_loss(q1, target_q1, delta = 50, reduction='none') 
                                      + q1_std *(q1_std_detach.pow(2) - 2*huber_loss(q1.detach(), target_q1_bound, delta = 50, reduction='none'))/(q1_std_detach +bias)
                            ))
        q2_loss = torch.mean((2*huber_loss(q2, target_q2, delta = 50, reduction='none')
        ...
        with torch.no_grad():
            # only Q mean loss 
            origin_q1_loss = torch.mean((2*huber_loss(q1, target_q1, delta = 50, reduction='none')
                                      )
                            )
            
            origin_q2_loss = torch.mean((2*huber_loss(q2, target_q2, delta = 50, reduction='none')
                                      )
                            )   
